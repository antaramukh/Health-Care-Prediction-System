...
def heart_predict():
    try:
        ...
        sc = StandardScaler()
        X = heart_data.iloc[:, [2, 7, 8, 9, 11, 12]].values
        sc.fit(X)
        ...
        prob_predict = heart_pipe.predict_proba(input_data_reshaped)[0][1] * 100
...
def liver_predict():
    try:
        liver_pipe = pickle.load(open("Liver.pkl", 'rb'))
...
def breastcancer_predict():
    try:
        ...
        breastcancer_pipe = pickle.load(open("Breast Cancer.pkl", 'rb'))
...
def kidney_predict():
    try:
        kidney_pipe = pickle.load(open("Kidney.pkl", 'rb'))
